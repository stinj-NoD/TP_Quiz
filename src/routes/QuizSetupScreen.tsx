...
import { useQuizStore, type NewQuizPlayerConfig } from '../store/quizStore'
...
function defaultPlayers(count: number, defaultName: string, defaultAgeLevel: AgeLevel): NewQuizPlayerConfig[] {
...
export function QuizSetupScreen() {
  ...
  const [players, setPlayers] = useState<NewQuizPlayerConfig[]>(
    defaultPlayers(1, profileName, profileAgeLevel),
  )
  const [rounds, setRounds] = useState(QUIZ_DEFAULT_ROUNDS)

  const usedColors = new Set(players.map((p) => p.color))

  const addPlayer = () => {
    if (players.length >= QUIZ_MAX_PLAYERS) return
  ...
  const removePlayer = () => {
    if (players.length <= QUIZ_MIN_PLAYERS) return
  ...
  return (
    <ScreenTransition>
      <ScreenHeader title="Mode Quiz" showBack={false} />
      <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-5 pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={players.length <= QUIZ_MIN_PLAYERS}
            ...
            <button
              type="button"
              onClick={addPlayer}
              disabled={players.length >= QUIZ_MAX_PLAYERS}
        ...
        <Card>
          <Slider
            label="Nombre de tours par joueur"
            value={rounds}
            min={QUIZ_MIN_ROUNDS}
            max={QUIZ_MAX_ROUNDS}
            onChange={setRounds}
