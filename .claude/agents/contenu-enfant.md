...
`difficulty` est OBLIGATOIRE sur chaque question générée (jamais omis). Pour le
segment enfant, privilégie très largement `"facile"` ; réserve `"moyen"` aux
questions qui demandent de croiser deux indices ou un peu plus de mémoire, et
n'utilise jamais `"difficile"` sur ce segment.
